...
$more = $more ?? '';
$offset = $offset ?? '';
$review_text_min = $review_text_min ?? '';
$review_text_max = $review_text_max ?? '';
...
$display_vicinity = $display_vicinity ?? NULL;
$display_avatar = $display_avatar ?? NULL;
$display_name = $display_name ?? FALSE;
$display_icon = $display_icon ?? FALSE;
$display_rating = $display_rating ?? FALSE;
$display_rating_stars = $display_rating_stars ?? FALSE;
$display_review_count = $display_review_count ?? FALSE;
$display_reviews = $display_reviews ?? FALSE;
$display_review_text = $display_review_text ?? FALSE;
$display_view_reviews_button = $display_view_reviews_button ?? FALSE;
$display_write_review_button = $display_write_review_button ?? FALSE;
$display_attribution = $display_attribution ?? FALSE;
$class_fill = $class_fill ?? FALSE;
$animate = $animate ?? FALSE;
$stylesheet = $stylesheet ?? FALSE;
...
?>
	<div class="<?php echo esc_attr($this->reference); ?>" data-limit="<?php echo esc_attr($limit); ?>">
        ...
        <p class="business-name"<?php echo (!$display_name && !$display_icon) ? ' style="display: none;"' : ''; ?>><?php echo (($this->business_icon != NULL) ? '<span class="icon"' . ((!$display_icon) ? ' style="display: none;"' : '') . '><img src="' . esc_attr($this->business_icon) . '" alt="' . esc_attr($this->business_name . ' ' . __('Icon', 'g-business-reviews-rating')) . '"></span>' : '') . '<span class="name"' . ((!$display_name) ? ' style="display: none;"' : '') . '>' . esc_html($this->business_name) . '</span>'; ?></p>
...
?>
        ...
        <p class="limit">
			...
			<input type="number" id="<?php echo esc_attr($this->get_field_id('limit')); ?>" class="small-text" name="<?php echo esc_attr($this->get_field_name('limit')); ?>" value="<?php echo esc_attr($limit); ?>" placeholder="—" step="1" min="0" max="<?php echo esc_attr($count); ?>">
			<span class="description"><a href="<?php echo esc_attr($this->plugin_url . '#reviews'); ?>"><?php
				/* translators: %1$s: number of reviews available */
				echo esc_html(sprintf(_n('%1$s review available', '%1$s reviews available', $count, 'g-business-reviews-rating'), $count));
			?></a></span>
        </p>
        <p class="view">
			<label for="<?php echo esc_attr($this->get_field_name('view')); ?>"><?php esc_attr_e('Carousel View:', 'g-business-reviews-rating'); ?></label>
			<input type="number" id="<?php echo esc_attr($this->get_field_id('view')); ?>" class="small-text view" name="<?php echo esc_attr($this->get_field_name('view')); ?>" value="<?php echo esc_attr((isset($view)) ? $view : ''); ?>" placeholder="—" step="1" min="1" max="<?php echo esc_attr(($count > 2) ? $count - 1 : 1); ?>">
            ...
            <input type="number" id="<?php echo esc_attr($this->get_field_id('interval')); ?>" class="small-text interval" name="<?php echo esc_attr($this->get_field_name('interval')); ?>" value="<?php echo esc_attr((isset($view) && isset($loop) && isset($interval) && is_numeric($view) && $loop && is_numeric($interval)) ? $interval : 7); ?>" placeholder="—" step="0.1" min="0.3" max="60">
...
<?php esc_html_e('Seconds per slide', 'g-business-reviews-rating'); ?></label>
        </p>
        <p class="offset">
			<label for="<?php echo esc_attr($this->get_field_name('offset')); ?>"><?php esc_attr_e('Review Offset:', 'g-business-reviews-rating'); ?></label>
			<input type="number" id="<?php echo esc_attr($this->get_field_id('offset')); ?>" class="small-text" name="<?php echo esc_attr($this->get_field_name('offset')); ?>" value="<?php echo esc_attr($offset); ?>" placeholder="—" step="1" min="0" max="<?php echo esc_attr(($count - 1)); ?>">
        </p>
        <p class="sort">
...
?>
        ...
        <p class="rating">
			...
			<input type="number" id="<?php echo esc_attr($this->get_field_id('rating_min')); ?>" class="small-text" name="<?php echo esc_attr($this->get_field_name('rating_min')); ?>" value="<?php echo esc_attr((is_numeric($rating_min) && $rating_min > 1) ? $rating_min : 1); ?>" step="1" min="1" max="5"> —
...
?>
        ...
        <p class="review-text-length">
			...
			<input type="number" id="<?php echo esc_attr($this->get_field_id('review_text_min')); ?>" class="small-text" name="<?php echo esc_attr($this->get_field_name('review_text_min')); ?>" value="<?php echo esc_attr($review_text_min); ?>" step="1" min="0" max="4000"> —
        ...
        <p class="excerpt-length">
			...
			<input type="number" id="<?php echo esc_attr($this->get_field_id('excerpt_length')); ?>" class="small-text" name="<?php echo esc_attr($this->get_field_name('excerpt_length')); ?>" value="<?php echo esc_attr($excerpt_length); ?>" placeholder="—" step="1" min="20" max="4000">
        </p>
        <p class="more-text">
